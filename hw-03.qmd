...
covid_survey_summary_stats <- covid_survey_summary_stats |>
  mutate(
    ...
    grouping = case_when(
      ...
      explanatory_value %in% c("Yes", "No") & explanatory == "exp_already_vax" ~ "Had COVID\nvaccine",
      explanatory_value %in% c("Yes", "No") & explanatory == "exp_flu_vax" ~ "Had flu\n vaccine this\nyear",
      TRUE ~ NA_character_
    ),
...
ggplot(covid_survey_summary_stats, aes(x = mean, y = explanatory_value)) +
  ...
  theme(
    ...
    strip.text.x = element_text(margin = margin(t = 4, r = 0, b = 10, l = 0))
  )
```

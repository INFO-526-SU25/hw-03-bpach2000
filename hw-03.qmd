...
covid_survey_summary_stats <- covid_survey_summary_stats |>
  mutate(
    ...
    grouping = case_when(
      ...
      explanatory_value %in% c("White", "Native Hawaiian / Other Pacific Islander", "Black / African American", "Asian", "American Indian / Alaskan Native") ~ "Race",
      ...
      explanatory_value %in% c("Yes", "No") & explanatory == "exp_already_vax" ~ "Had COVID Vaccine",
      explanatory_value %in% c("Yes", "No") & explanatory == "exp_flu_vax" ~ "Had flu Vaccine this year",
      TRUE ~ NA_character_
    ),
    grouping = factor(grouping, levels = c("All", "Age", "Gender", "Race", "Ethnicity", "Profession", "Had COVID Vaccine", "Had flu Vaccine this year")),
    explanatory_value = factor(explanatory_value, levels = c("<20", "21-25", "26-30", ">30", "", "Female", "Male", "Non-binary/Third gender", "Prefer not to say", "American Indian / Alaskan Native", "Asian", "Black / African American", "Native Hawaiian / Other Pacific Islander", "White", "Hispanic / Latino", "Non-Hispanic / Non-Latino", "Medical", "Nursing", "No", "Yes")),
  )

ggplot(covid_survey_summary_stats, aes(x = mean, y = explanatory_value)) +
  geom_point(size = 0.6) +
  geom_errorbarh(aes(xmin = low, xmax = high), height = 0.5, color = "black", size = 0.3) +
  facet_grid(grouping ~ response, scales = "free_y", space = "free_y") +
